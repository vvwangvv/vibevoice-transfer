...
class SConvTranspose1d(nn.Module):
    ...
    def __init__(
        ...
        self.convtr = NormConvTranspose1d(
            in_channels, out_channels, kernel_size, stride, causal=causal, norm=norm, norm_kwargs=norm_kwargs, bias=bias
        )
        self.causal = causal
        ...
        for i in range(len(self.depths)):
            ...
            for block in self.stages[i]:
                if hasattr(block, "mixer") and hasattr(block.mixer, "conv") and isinstance(block.mixer.conv, SConv1d):
                    ...
                    x = block.norm(x)
                    x = block.mixer.conv(
                        x, cache=cache, sample_indices=sample_indices, use_cache=use_cache, debug=debug
                    )
...
class VibeVoiceAcousticTokenizerModel(PreTrainedModel):
    ...
    def forward(self, audio, cache=None, sample_indices=None, use_cache=False, debug=False):
        ...
        reconstructed = self.decode(
            sampled_latents, cache=cache, sample_indices=sample_indices, use_cache=use_cache, debug=debug
        )
