...
from vibevoice.finetune.data_vibevoice import _load_audio_from_tar
...
class VibeVoiceTokenizerProcessor(FeatureExtractionMixin):
    ...
    def _load_audio_from_path(self, audio_path: str) -> np.ndarray:
        ...
        if file_ext in [".wav", ".mp3", ".flac", ".m4a", ".ogg"]:
            ...
            # Tar entry (i.e. xxx.tar:offset:size)
            audio_tensor, sr = _load_audio_from_tar(audio_path)
            audio_array = audio_tensor.mean(dim=0).numpy()
            audio_array = _resample_if_needed(audio_array, sr, self.sampling_rate)
            return audio_array
        elif file_ext == ".pt":
            # PyTorch tensor file
